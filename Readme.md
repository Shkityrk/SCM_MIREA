...
Этот проект представляет собой Python-приложение, предназначенное для преобразования конфигурационных файлов JSON в текстовое представление в специфическом языке конфигурации. Программа принимает JSON-файл в качестве входных данных, парсит его и генерирует выходные данные в соответствии с заданными правилами.

## Структура проекта

```
D:.
│   imports.py       # Содержит все необходимые импорты для работы приложения.
│   main.py          # Главный скрипт, который принимает JSON-файл в качестве 
│                    # входных данных, парсит его и генерирует выходные данные 
│                    # в требуемом формате.
│
└───src
    │   __init__.py
    │
    ├───env
    │   <file>       # Пример конфигурационного JSON-файла, который будет парситься и преобразовываться.
    │
    ├───tests
    │       generator.py   # Юнит-тесты для логики генерации выхода.
    │       test.py        # Юнит-тесты для общей логики парсинга и обработки ошибок.
    │    
    │
    └────utils
            errors.py         # Содержит обработку ошибок для недопустимых типов данных.
            generator.py      # Отвечает за генерацию итогового вывода в требуемом формате языка конфигурации.
            parser.py
            __init__.py
```
## Использование
...
1. Запустите скрипт `main.py`:
   ...
   python main.py
   ```
2. Вас попросят ввести путь к JSON-файлу, который вы хотите преобразовать.
3. Скрипт выведет преобразованный текст в формате учебного конфигурационного языка.

Пример:
```
Введите путь к JSON-файлу: ./src/env/<name_your_file>.json
Сконвертированный текст в учебном конфигурационном языке:
begin
 key1 := 'value1';
 key2 := 123;
end
...
В проекте есть юнит-тесты для функций генератора и парсера:

- **TestGenerator**: Тестирует генерацию выходных данных из парсированного ввода.
- **TestParser**: Тестирует парсинг JSON-данных и обработку ошибок, таких как недопустимые типы данных.
...
В приложении используется обработка ошибок с помощью класса `ConfigurationError`. Это гарантирует, что недопустимые типы данных (например, неподдерживаемые типы в JSON) будут правильно обнаружены и сообщены.
